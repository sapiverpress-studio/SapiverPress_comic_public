...
const STATE_PATH = "indie-publishing-tip-pinterest-posts.json";
const DEFAULT_LINK = "https://sapiverpress.etsy.com";
const DEFAULT_BOARD_NAME = "Sapiver Press Indie Publishing Tips";
...
async function createBoard(name) {
  return pinterestFetch("/boards", {
    ...
    body: JSON.stringify({
      name,
      description: "Daily indie publishing tips from Sapiver Press."
    })
  });
}
...
async function main() {
  ...
  const mode = String(process.env.INDIE_TIP_POST_MODE || "live").trim().toLowerCase();
  ...
  const manifestPath = `social/indie-publishing-tips/${date}/manifest.json`;
  const manifest = await readJson(manifestPath, null);
  if (!manifest) throw new Error(`Missing indie publishing daily tip manifest: ${manifestPath}`);
  ...
  const payload = {
    board_id: board.id,
    title: String(manifest.title || "Indie Publishing Tip").slice(0, 100),
    description: String(manifest.caption || manifest.tip || "Daily indie publishing tip from Sapiver Press.").slice(0, 500),
    link: process.env.PINTEREST_LINK_URL || manifest.link_url || DEFAULT_LINK,
    alt_text: String(manifest.alt_text || manifest.tip || "Sapiver Press indie publishing tip card.").slice(0, 500),
    media_source: {
      source_type: "image_base64",
      content_type: "image/png",
      data: await readBase64(imageRel)
    }
  };
  ...
  console.log(`Pinterest daily indie publishing tip created for ${date}: ${state[date].pin_id || "unknown pin id"}`);
}
