...
async function fetchPuzzleState(date) {
  ...
  for (const candidate of localCandidates) {
    try {
      ...
      return { ...fallback, fetch_status: "local_capture_json", mode: data.mode || data.source_name || "Trigoku", source_id: data.source_id || "trigoku", summary: `Local capture data found: ${data.mode || data.source_name || "Trigoku"}.`, variant_recap: data.variant_recap || null };
    } catch {}
  }
...
async function callClaude({ date, weekdayName, characterFile, storyHistory, puzzleState, setting }) {
  ...
  const payload = { date, weekday: weekdayName, required_output_shape: storylineSchemaDescription(), allowed_pose_ids: ALLOWED_POSES, required_beat_order: sceneSkeleton().map((s) => ({ pose_id: s.pose_id, beat: s.beat })), locked_visual_traits: characterFile.locked_visual_traits || ISLA_CHARACTER.locked_visual_traits, caption_bank: characterFile.caption_bank || FALLBACK_CAPTIONS, selected_setting: setting, puzzle_state: puzzleState, variant_recap: puzzleState?.variant_recap || null, last_7_story_history: recentSeven, last_3_story_history_to_avoid_repeating: recentThree, hard_rules: ["Each scene_description must be 2 sentences max.", "Each caption must be short, specific, real, and not promotional.", "At least two panels must imply front-facing or three-quarter face visibility through image_prompt_fragment.", `facebook_post_text must end exactly with ${SUITE_URL}`] };
  const response = await fetch(ANTHROPIC_URL, { method: "POST", headers: { "content-type": "application/json", "x-api-key": apiKey, "anthropic-version": "2023-06-01" }, body: JSON.stringify({ model: ANTHROPIC_MODEL, max_tokens: 2200, temperature: 0.85, system, messages: [{ role: "user", content: JSON.stringify(payload, null, 2) }] }) });
...
function normaliseStory({ generated, date, weekdayName, characterFile, storyHistory, puzzleState, setting, source }) {
  ...
  const scenes = skeleton.map((skel, index) => {
    ...
    return { id: skel.id, title: skel.title, beat: skel.beat, pose_id: pose, scene_description: String(raw.scene_description || skel.beat).slice(0, 260), caption: shortCaption(raw.caption, captions[index] || FALLBACK_CAPTIONS[index]), speech_bubble: String(raw.speech_bubble || "").replace(/\s+/g, " ").trim().slice(0, 90), dialogue: String(raw.speech_bubble || "").replace(/\s+/g, " ").trim().slice(0, 90), image_prompt_fragment: fragment, full_image_prompt: buildFullImagePrompt(fragment, setting, index), setting, view_rule: viewRule(index), image_ref: imageRef(skel.id), screen_state: skel.id };
  });
  ...
  const story = { date, weekday: weekdayName, character_id: "isla", character_name: "Isla", trigger_word: "ISLA_SP", render_mode: "illustrated_comic_panels", product_referenced: productReference(), puzzle_state: puzzleState, selected_setting: setting, story_source: source, story_note: String(generated?.story_note || `Isla returns to the ${puzzleState.difficulty_phase} Trigoku lock.`).slice(0, 260), continuation_note: String(generated?.continuation_note || "Fallback rhythm used; future Claude runs should vary from recent history.").slice(0, 260), facebook_post_text: facebookPostText, scenes, history_used_count: storyHistory.length };
...
export async function runLocalDailyGeneration() {
  ...
  let generated;
  let source = "claude";
  try { generated = await callClaude({ date, weekdayName, characterFile, storyHistory: withoutToday, puzzleState, setting }); } catch (error) { console.error("Claude storyline generation failed. Writing varied local fallback story."); console.error(error?.message || error); generated = fallbackGenerated(date); source = "fallback"; }
  const story = normaliseStory({ generated, date, weekdayName, characterFile, storyHistory: withoutToday, puzzleState, setting, source });
