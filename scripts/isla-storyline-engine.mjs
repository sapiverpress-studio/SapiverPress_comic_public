  ...
  return [
    {
      id: "scene_01",
      title: "Back Again",
      pose_id: "pose_01_back_again",
      beat: "Isla arrives / opens the grid; calm, returning.",
    },
    {
      id: "scene_02",
      title: "First Moves",
      pose_id: "pose_02_first_moves",
      beat: "First moves; early engagement.",
    },
    {
      id: "scene_03",
      title: "Resistance",
      pose_id: "pose_03_stuck",
      beat: "A moment of resistance or hesitation, varied each day.",
    },
    {
      id: "scene_04",
      title: "Thinking It Through",
      pose_id: "pose_04_thinking",
      beat: "Thinking through it; a shift in approach.",
    },
    {
      id: "scene_05",
      title: "Almost There",
      pose_id: "pose_05_coffee",
      beat: "Near completion or a short reset moment.",
    },
    {
      id: "scene_06",
      title: "Looking Ahead",
      pose_id: "pose_06_leaving",
      beat: "Wrapping up and looking ahead.",
    },
  ];
}
...
function imageRef(sceneId) {
  const names = {
    ...
    scene_02: "first_move",
    scene_03: "stuck_moment",
    scene_04: "breakthrough",
    scene_05: "finish",
    scene_06: "tomorrow_set",
  };
...
function productReference() {
  return {
    key: "trigoku_daily_lock",
    name: "Trigoku Daily Lock",
    url: SUITE_URL,
    natural_reference: "today's Trigoku lock",
  };
}
...
async function fetchPuzzleState(date) {
  const fallback = {
    product_name: "Trigoku Daily Lock",
    product_url: SUITE_URL,
    play_url: TRIGOKU_URL,
    date,
    difficulty_phase: inferPuzzlePhase(date),
    fetch_status: "inferred_from_date",
    summary: "Live Trigoku page data was not available before capture; using date-based early/mid/late week difficulty.",
  };

  const localCandidates = [
    path.join(ROOT, "captures", date, "extracted", "today_trigoku_data.json"),
    path.join(ROOT, "captures", date, "extracted", "today_puzzle_data.json"),
    path.join(ROOT, "today_trigoku_data.json"),
  ];
  for (const candidate of localCandidates) {
    try {
      ...
      return {
        ...fallback,
        fetch_status: "local_capture_json",
        mode: data.mode || data.source_name || "Trigoku",
        source_id: data.source_id || "trigoku",
        summary: `Local capture data found: ${data.mode || data.source_name || "Trigoku"}.`,
      };
    } catch {}
  }
  ...
  for (const url of [TRIGOKU_URL, "https://sapiverpress-trigoku.netlify.app/"]) {
    try {
      ...
      return {
        ...fallback,
        fetch_status: "live_page_reachable",
        source_url: url,
        page_title: title || null,
        difficulty_label: difficulty || null,
        summary: `Live Trigoku page reachable${title ? `: ${title}` : ""}.`,
      };
    } catch {}
  }
...
function pickSetting(date, characterFile) {
  const settings = Array.isArray(characterFile.settings) && characterFile.settings.length
    ? characterFile.settings
    : ISLA_CHARACTER.settings;
  return settings[stableIndex(date, settings.length)];
}

function viewRule(index) {
  if (index === 0 || index === 3) return "front-facing or three-quarter view, Isla's face clearly visible";
  return "natural varied camera angle, not a repeated pose";
}
...
function buildFullImagePrompt(fragment, setting, index) {
  ...
  return [
    `ISLA_SP ${cleanFragment}`,
    "young Black woman warm medium brown skin light freckles on nose and cheeks",
    "natural coily dark hair high voluminous puff bun",
    "wide floral teal rust orange headband worn across forehead",
    "medium gold hoop earrings",
    "oversized deep teal hoodie",
    viewRule(index),
    setting,
    ...
    "gothic window cityscape view",
    "ivy bookshelf",
    "warm painterly editorial illustration style",
    "cinematic warm amber palette",
    "detailed textures",
  ].join(", ");
}
...
function storylineSchemaDescription() {
  return {
    story_note: "short summary of today's arc",
    continuation_note: "how today differs from the last few days",
    facebook_post_text: `2 sentences max, Isla's voice, must end with ${SUITE_URL}`,
    beats: [
      {
        scene_description: "2 sentences max",
        pose_id: ALLOWED_POSES[0],
        caption: "short understated caption",
        speech_bubble: "optional short thought",
        image_prompt_fragment: "emotion/action only",
      },
    ],
  };
}

async function callClaude({ date, weekdayName, characterFile, storyHistory, puzzleState, setting }) {
  ...
  const payload = {
    date,
    weekday: weekdayName,
    required_output_shape: storylineSchemaDescription(),
    allowed_pose_ids: ALLOWED_POSES,
    required_beat_order: sceneSkeleton().map((s) => ({ pose_id: s.pose_id, beat: s.beat })),
    locked_visual_traits: characterFile.locked_visual_traits || ISLA_CHARACTER.locked_visual_traits,
...
function normaliseStory({ generated, date, weekdayName, characterFile, storyHistory, puzzleState, setting, source }) {
  ...
  const scenes = skeleton.map((skel, index) => {
    ...
    const fragment = raw.image_prompt_fragment || fallbackFragment(index, date);
    return {
      id: skel.id,
      title: skel.title,
      beat: skel.beat,
      pose_id: pose,
      scene_description: String(raw.scene_description || skel.beat).slice(0, 260),
      caption: shortCaption(raw.caption, captions[index] || FALLBACK_CAPTIONS[index]),
      speech_bubble: String(raw.speech_bubble || "").replace(/\s+/g, " ").trim().slice(0, 90),
      dialogue: String(raw.speech_bubble || "").replace(/\s+/g, " ").trim().slice(0, 90),
      image_prompt_fragment: fragment,
      full_image_prompt: buildFullImagePrompt(fragment, setting, index),
      setting,
      view_rule: viewRule(index),
      image_ref: imageRef(skel.id),
      screen_state: skel.id,
    };
  });
  ...
  if (!facebookPostText.endsWith(SUITE_URL)) {
    facebookPostText = `${facebookPostText.replace(SUITE_URL, "").trim()} ${SUITE_URL}`.trim();
  }
  ...
  const story = {
    ...
    selected_setting: setting,
    story_source: source,
    story_note: String(generated?.story_note || `Isla returns to the ${puzzleState.difficulty_phase} Trigoku lock.`).slice(0, 260),
    continuation_note: String(generated?.continuation_note || "Fallback rhythm used; future Claude runs should vary from recent history.").slice(0, 260),
    facebook_post_text: facebookPostText,
    scenes,
    history_used_count: storyHistory.length,
  };
...
function fallbackFragment(index, date) {
  ...
  return [
    "opening the daily grid calmly, returning to her familiar puzzle ritual",
    "leaning closer with pencil near open journal, making the first careful moves",
    `pausing over ${focus}, thoughtful but not frustrated`,
    "thinking through a clean deduction, small shift in expression as the route opens",
    "taking a quiet coffee reset beside the nearly complete grid",
    "packing up calmly after the finished grid, small satisfied smile",
  ][index];
}
...
function fallbackGenerated(date) {
  ...
  return {
    ...
    beats: sceneSkeleton().map((scene, index) => ({
      ...
      speech_bubble: index === 2 ? "No guessing." : "",
      image_prompt_fragment: fallbackFragment(index, date),
    })),
  };
}
...
function imageManifest(characterFile, story) {
  return {
    ...
    trigger_word: "ISLA_SP",
    render_mode: "illustrated_comic_panels",
    required_character_files: story.scenes.map((s) => `templates/characters/isla/${s.image_ref}`),
    text_is_overlay: true,
    puzzle_screen_inserted_later: true,
    puzzle_product: "Trigoku Daily Lock",
    puzzle_url: SUITE_URL,
    selected_setting: story.selected_setting,
    image_prompts: story.scenes.map((s) => ({
      scene: s.id,
      pose_id: s.pose_id,
      prompt: s.full_image_prompt,
    })),
    style_rules: [
      "Isla is the only active daily comic character.",
      "Use ISLA_SP trigger word in every image prompt.",
      "Young Black woman, warm medium brown skin, light freckles on nose and cheeks.",
      "Natural coily dark hair in high voluminous puff bun.",
      "Wide floral headband in teal, rust orange, cream, and dusty blue, worn across forehead.",
      "Medium gold hoop earrings.",
      "Oversized deep teal hoodie.",
      "At least 2 of the 6 panels must be front-facing or three-quarter view showing Isla's face clearly.",
      "Puzzle screen must come from a real captured puzzle page.",
      ...(Array.isArray(characterFile.style_rules) ? characterFile.style_rules : []),
    ],
    compositor_rules: [
      "Character art first.",
      "Puzzle screen composited second.",
      "Caption bar text overlaid last.",
...
function buildCharacterFile(previous, storyHistory) {
  return {
    ...
    trigger_word: "ISLA_SP",
    visual_identity: {
      age_read: "young adult",
      skin: "warm medium brown skin",
      freckles: "light freckles on nose and cheeks",
      hair: "natural coily dark hair in high voluminous puff bun",
      headband: "wide floral headband — teal, rust orange, cream, dusty blue — worn across forehead",
      earrings: "medium gold hoop earrings",
      clothing: "oversized deep teal hoodie",
    },
    locked_visual_traits: ISLA_CHARACTER.locked_visual_traits,
    poses: sceneSkeleton().map((scene) => ({
      pose_id: scene.pose_id,
      title: scene.title,
      purpose: scene.beat,
    })),
    settings: ISLA_CHARACTER.settings,
    brand_props: ISLA_CHARACTER.brand_props,
    caption_bank: [
      "Back again.",
      "One number at a time.",
      "Start small.",
    ...
    story_rules: [
      "Isla posts every day, not only Tuesdays.",
      "Isla is the only character.",
      "The story is about Isla living her life with a Trigoku Daily Lock subplot; the puzzle is not the main character.",
      "Keep captions short, understated, real, and non-promotional.",
      "Avoid repeating the same arc, same stuck moment, or same visual rhythm across recent days.",
      "At least two panels must be front-facing or three-quarter view showing Isla's face clearly.",
      "Never invent puzzle content; real puzzle captures are composited later.",
    ],
    story_history: storyHistory.slice(-30),
    weeks: storyHistory.slice(-30),
    ongoing_threads: Array.isArray(previous.ongoing_threads) ? previous.ongoing_threads : [],
    last_updated: storyHistory.at(-1)?.date || previous.last_updated || null,
  };
}
...
export async function runLocalDailyGeneration() {
  ...
  const existingCharacter = await readJson("characters/isla.json", {
    character_id: "isla",
    name: "Isla",
    story_history: [],
    weeks: [],
  ...
  const previousHistory = Array.isArray(existingCharacter.story_history)
    ? existingCharacter.story_history
    : Array.isArray(existingCharacter.weeks)
      ? existingCharacter.weeks.map((entry) => ({
          date: entry.date,
          weekday: entry.weekday,
          story_note: entry.story_note,
  ...
  try {
    generated = await callClaude({
      ...
      weekdayName,
      characterFile,
      storyHistory: withoutToday,
      puzzleState,
      setting,
    });
  } catch (error) {
    console.error("Claude storyline generation failed. Writing varied local fallback story.");
    console.error(error?.message || error);
    generated = fallbackGenerated(date);
    source = "fallback";
  }
