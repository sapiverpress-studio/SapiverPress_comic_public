...
if (MODE === "live" && alreadyPosted && !force) { console.log(`YouTube upload already has a live result for ${DATE}; skipping duplicate upload. Set FORCE_YOUTUBE_POST=1 to override.`); process.exit(0); }
if (MODE !== "live") { out.dry_run = true; out.note = "Prepared YouTube Shorts upload. Set YOUTUBE_POST_MODE=live and add YouTube OAuth secrets to upload."; }
else { const missing = missingYouTubeSecrets(); if (missing.length) { out.skipped = true; out.missing_secrets = missing; out.note = "YouTube upload skipped because required GitHub secrets are missing."; console.warn(out.note, missing.join(", ")); } else { out.dry_run = false; const accessToken = await refreshAccessToken(); out.youtube = await uploadYouTubeVideo({ accessToken, videoPath, title, description }); out.youtube_url = out.youtube?.id ? `https://www.youtube.com/watch?v=${out.youtube.id}` : null; } }
await fs.mkdir(path.dirname(RESULT), { recursive: true });
await fs.writeFile(RESULT, JSON.stringify(out, null, 2) + "\n", "utf8");
console.log(`${out.youtube ? "Posted" : out.skipped ? "Skipped" : "Prepared"} Book Content OS YouTube Short for ${DATE}`);
