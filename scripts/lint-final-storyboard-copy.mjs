...
const BANNED_WORDS = ["quiet", "gentle", "pause", "thread", "anchor", "ritual", "borrowed", "understated", "restraint"];
...
const REPLACEMENTS = [
  ...
  [/\bTrigoku constraint\b/gi, "daily rule"],
...
function realVariantName(story) { const name = clean(story?.variant_recap?.variant_name || story?.image_manifest?.variant_recap?.variant_name || story?.puzzle_state?.source_name || story?.puzzle_state?.mode || ""); if (!name || /^col$/i.test(name)) return ""; return name && name.toLowerCase() !== "trigoku" ? name : ""; }
function realVariantLine(story) { const line = clean(story?.variant_recap?.line || story?.variant_recap?.short_rule || story?.image_manifest?.variant_recap?.line || story?.image_manifest?.variant_recap?.short_rule || ""); return line && !/little rule-set|check the constraint before rushing/i.test(line) ? sanitise(line) : ""; }
...
function isPuzzleRuleText(text) { return /check (the )?(daily rule|variant rule)|rule changes the move|before trusting it|before trusting the answer/i.test(clean(text)); }
function fallbackDialogue(index) { return ["One thing first.", "Answer it properly later.", "Start where it proves itself.", "Check the daily rule before trusting it.", "That one holds.", "Leave it there. Move on."][index] || "Keep going."; }
...
function refreshStoryLedLocationFlow(story) { story.storyboard_locations = (story.scenes || []).slice(0, 6).map((s) => s.panel_location || s.setting || ""); story.location_flow_validated = true; story.location_flow_changed = false; story.location_flow = (story.scenes || []).slice(0, 6).map((s) => s.location_key || s.scene_truth_flow_id || s.panel_location || s.setting || ""); story.story_led_location_flow_preserved = true; return story; }
...
function syncManifest(story, manifest = {}) { manifest.variant_recap = story.variant_recap || manifest.variant_recap || null; manifest.variant_copy_mode = story.variant_copy_mode; manifest.variant_detection_unresolved = story.variant_detection_unresolved; manifest.storyboard_arc = story.storyboard_arc; manifest.storyboard_quality = story.storyboard_quality; manifest.storyboard_arc_title = story.storyboard_arc_title; manifest.final_storyboard_lint = story.final_storyboard_lint; manifest.copy_repetition_lint = story.copy_repetition_lint; manifest.final_copy_sanity = story.final_copy_sanity; manifest.location_flow_validated = story.location_flow_validated; manifest.location_flow = story.location_flow; manifest.storyboard_locations = story.storyboard_locations; manifest.storyboard_copy_source = story.storyboard_copy_source; manifest.gemini_allowed_fields = story.gemini_allowed_fields; manifest.gemini_preserved_locations = story.gemini_preserved_locations; manifest.gemini_preserved_panel_order = story.gemini_preserved_panel_order; manifest.post_ready_contract = story.post_ready_contract; manifest.scene_truth_contract = story.scene_truth_contract || manifest.scene_truth_contract || {}; manifest.story_continuity = story.story_continuity || manifest.story_continuity || {}; manifest.supporting_life_trigger = story.supporting_life_trigger || manifest.supporting_life_trigger || {}; manifest.image_prompts = (story.scenes || []).slice(0, 6).map((scene, index) => ({ ...(manifest.image_prompts?.[index] || {}), scene: scene.id, pose_id: scene.pose_id, caption: scene.caption, dialogue: scene.dialogue, storyboard_caption: scene.storyboard_caption, storyboard_dialogue: scene.storyboard_dialogue, panel_location: scene.panel_location || scene.setting || "", panel_action: scene.panel_action || "", panel_pose_family: scene.panel_pose_family || "", panel_screen_state: scene.panel_screen_state || "" })); return manifest; }
function buildPostReadyContract(story) { const q = story.storyboard_quality || {}; return { openai_ok: story.openai_storyboard_status === "ok" || String(story.storyboard_copy_source || "").startsWith("openai"), quality_gate_action: story.quality_gate_action || q.quality_gate_action || "unknown", gemini_preserved_locations: story.gemini_preserved_locations !== false, final_copy_sanity_passed: Boolean(story.final_copy_sanity?.passed), location_flow_validated: Boolean(story.location_flow_validated), all_panels_have_screen_overlay: true, variant_copy_mode: story.variant_copy_mode || "neutral_daily_rule", posting_allowed: Boolean(story.final_copy_sanity?.passed) && (story.storyboard_quality?.final_lint_passed !== false) }; }
...
async function main() {
  ...
  story = refreshStoryLedLocationFlow(story);
  ...
  console.log(`Story-led location flow preserved: ${story.story_led_location_flow_preserved ? "yes" : "no"}`);
}
