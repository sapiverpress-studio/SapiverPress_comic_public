...
function altTextFor(manifest) {
  ...
  return `${title}: illustrated story panel showing Isla planning a Sudoku book publishing workflow with paper folders and notebook.`.slice(0, 500);
}
...
async function main() {
  ...
  if (!token || !boardId) {
    console.log("Pinterest post skipped: missing PINTEREST_ACCESS_TOKEN or PINTEREST_BOARD_ID.");
    return;
  }
  ...
  state[date] = { posted_at: new Date().toISOString(), pin_id: body.id || body.pin_id || null, image: imageRel, title: payload.title, link: payload.link };
