...
const MODE = (process.env.BOOK_CONTENT_OS_PINTEREST_VIDEO_POST_MODE || "live").toLowerCase();
...
function cleanTitle(file) { return path.basename(String(file || ""), path.extname(String(file || ""))).replace(/^\d+[_-]?/, "").replace(/^BCOS[_-]SET[_-]\d+[_-]?/i, "").replace(/[_-]+/g, " ").replace(/\bpngs?\b/gi, "").replace(/\s+/g, " ").trim(); }
function toPositiveInteger(value, fallback) { if (String(value || "").trim().toLowerCase() === "all") return Number.MAX_SAFE_INTEGER; const n = Number.parseInt(String(value || ""), 10); return Number.isFinite(n) && n >= 0 ? n : fallback; }
...
function uniqueList(values) { return [...new Map((values || []).filter(Boolean).map((v) => [String(v).toLowerCase(), v])).values()]; }
function imagePinSources(manifest) { const raw = manifest.facebook?.images?.length ? manifest.facebook.images : manifest.pinterest_video?.source_images?.length ? manifest.pinterest_video.source_images : [manifest.pinterest?.image].filter(Boolean); const images = uniqueList(raw); const limit = Math.min(images.length, toPositiveInteger(process.env.PINTEREST_IMAGE_PIN_LIMIT, DEFAULT_IMAGE_PIN_LIMIT)); return images.slice(0, limit); }
function imagePinTitle(baseTitle, relImage, index, total) { const imageTitle = cleanTitle(relImage); const suffix = imageTitle || `image ${index + 1}`; const count = total > 1 ? ` ${index + 1}/${total}` : ""; const title = `Book Content OS image${count} | ${suffix}`; return title.trim().slice(0, 100) || String(baseTitle || "Book Content OS image").trim().slice(0, 100); }
function existingImagePin(existing, sourceImage) { const sourceKey = String(sourceImage || "").toLowerCase(); return (existing?.image_pins?.pins || []).find((item) => String(item?.source_image || "").toLowerCase() === sourceKey && item?.pin?.id); }
async function postImageToPinterest({ token, boardId, imagePath, title, description }) { return jsonFetch("https://api.pinterest.com/v5/pins", { method: "POST", headers: { Authorization: `Bearer ${token}`, "Content-Type": "application/json" }, body: JSON.stringify({ board_id: boardId, title: title.trim().slice(0, 100), description: pinterestDescription(description), media_source: { source_type: "image_base64", content_type: "image/png", data: b64(imagePath) } }) }); }
async function uploadVideoToPinterest({ token, boardId, videoPath, title, description }) {
  const media = await jsonFetch("https://api.pinterest.com/v5/media", { method: "POST", headers: { Authorization: `Bearer ${token}`, "Content-Type": "application/json" }, body: JSON.stringify({ media_type: "video" }) });
  if (!media.media_id || !media.upload_url) throw new Error(`Pinterest media registration did not return media_id and upload_url: ${JSON.stringify(media).slice(0, 1600)}`);
  ...
  let lastStatus = null;
  for (let attempt = 1; attempt <= 24; attempt++) { await sleep(10000); const status = await jsonFetch(`https://api.pinterest.com/v5/media/${media.media_id}`, { headers: { Authorization: `Bearer ${token}` } }); lastStatus = status; const state = String(status.status || status.upload_status || "").toLowerCase(); if (["succeeded", "success", "available", "ready"].includes(state)) break; if (["failed", "failure", "rejected"].includes(state)) throw new Error(`Pinterest video processing failed: ${JSON.stringify(status).slice(0, 1600)}`); }
  const pin = await jsonFetch("https://api.pinterest.com/v5/pins", { method: "POST", headers: { Authorization: `Bearer ${token}`, "Content-Type": "application/json" }, body: JSON.stringify({ board_id: boardId, title: title.trim().slice(0, 100), description: pinterestDescription(description), media_source: { source_type: "video_id", media_id: media.media_id, cover_image_key_frame_time: 0 } }) });
...
const title = await readOptional(manifest.pinterest_video.title, "Book Content OS Lite + Pro video");
...
const out = { date: DATE, mode: MODE, type: "book_content_os_pinterest_image_pins_and_video_post_v1", cta_text: manifest.cta_text || "link in bio", board_id: boardId, board_name: DEFAULT_PINTEREST_BOARD_NAME, video: manifest.pinterest_video.video, source_images: manifest.pinterest_video.source_images || [], image_pin_sources: imageSources, title, caption, first_comment: firstComment };
if (MODE !== "live") {
  out.dry_run = true;
