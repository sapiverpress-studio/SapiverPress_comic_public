...
const ANECDOTE_CANDIDATES = [
  path.join(ROOT, "content", "publishing-anecdotes-daily-posts.csv"),
  path.join(ROOT, "copy", "sapiver_press_anecdotes.csv"),
  path.join(ROOT, "content", "sapiver_press_anecdotes.csv")
];
const ASSET_ROOTS = [
...
function normaliseFact(row) {
  return { id: String(row.id || row.day || row.fact || "").trim(), text: String(row.fact || row.copy || row.text || row.anecdote || "").trim(), source: "publishing_facts", raw: row };
}
function normaliseAbout(row) {
  return { id: String(row.id || row.angle || row.copy || "").trim(), text: String(row.copy || row.text || "").trim(), audience: row.audience || "", angle: row.angle || "", use_case: row.use_case || "", primary_url: row.primary_url || "", secondary_url: row.secondary_url || "", source: "about_copy", raw: row };
}
function normaliseAnecdote(row) {
  return { id: String(row.id || row.day || row.title || row.copy || row.text || row.anecdote || "").trim(), text: String(row.anecdote || row.copy || row.text || row.note || row.fact || "").trim(), source: "anecdotes", raw: row };
}
...
function copyList(images, dir) { fssync.mkdirSync(dir, { recursive: true }); return images.map((src, i) => { const dst = path.join(dir, `${String(i + 1).padStart(2, "0")}_${path.basename(src)}`); fssync.copyFileSync(src, dst); return rel(dst); }); }
function ffmpegEscape(file) { return file.replace(/'/g, "'\\''"); }
function buildVideo(images, dir) { fssync.mkdirSync(dir, { recursive: true }); const copied = copyList(images, dir); const copiedAbs = copied.map((p) => path.join(ROOT, p)); const listPath = path.join(dir, "ffmpeg-input.txt"); const lines = []; for (const img of copiedAbs) { lines.push(`file '${ffmpegEscape(img)}'`); lines.push(`duration ${VIDEO_SECONDS_PER_IMAGE}`); } lines.push(`file '${ffmpegEscape(copiedAbs[copiedAbs.length - 1])}'`); fssync.writeFileSync(listPath, `${lines.join("\n")}\n`, "utf8"); const videoPath = path.join(dir, "pinterest_video.mp4"); execFileSync("ffmpeg", ["-y", "-f", "concat", "-safe", "0", "-i", listPath, "-vf", "scale=1080:1920:force_original_aspect_ratio=decrease,pad=1080:1920:(ow-iw)/2:(oh-ih)/2,format=yuv420p", "-r", "30", "-c:v", "libx264", "-pix_fmt", "yuv420p", "-movflags", "+faststart", videoPath], { stdio: "inherit" }); return { images: copied, video: rel(videoPath) }; }
