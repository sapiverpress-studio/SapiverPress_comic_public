...
async function writeJson(filePath, data) { await ensureDir(path.dirname(filePath)); await fs.writeFile(filePath, `${JSON.stringify(data, null, 2)}\n`, "utf8"); }
...
function fetchErrorDetails(error) {
  ...
  return { message: error?.message || String(error), name: error?.name || "Error", cause_message: cause?.message || "", cause_code: cause?.code || "", cause_errno: cause?.errno || "", cause_syscall: cause?.syscall || "", cause_hostname: cause?.hostname || cause?.host || "", cause_address: cause?.address || "", cause_port: cause?.port || "" };
}
...
function loraDescriptor() { return { path: LORA_URL, scale: LORA_SCALE }; }
function falInput(prompt, negativePrompt) { return { prompt, image_size: { width: WIDTH, height: HEIGHT }, num_inference_steps: STEPS, guidance_scale: GUIDANCE, num_images: 1, enable_safety_checker: true, output_format: "png", loras: [loraDescriptor()], negative_prompt: negativePrompt }; }
function hfFallbackBody(prompt, negativePrompt) { return { inputs: prompt, parameters: { width: WIDTH, height: HEIGHT, num_inference_steps: Math.min(STEPS, 30), guidance_scale: Math.max(GUIDANCE, 7.0), negative_prompt: negativePrompt }, options: { wait_for_model: true, use_cache: false } }; }
...
async function preflightHf(summary) {
  if (!HF_TOKEN) { summary.preflight.hf_token = { ok: false, reason: "HF_TOKEN missing" }; return; }
  try {
    const response = await fetchWithDiagnostics("https://huggingface.co/api/whoami-v2", { method: "GET", headers: { Authorization: `Bearer ${HF_TOKEN}` } }, 30000);
    const body = await parseJsonResponse(response);
    summary.preflight.hf_token = { ok: response.ok, status: response.status, status_text: response.statusText, name: body.json?.name || body.json?.auth?.accessToken?.displayName || "", error: response.ok ? "" : body.text.slice(0, 600) };
    console.log(`HF token preflight: ${response.status} ${response.statusText}`);
  } catch (error) { summary.preflight.hf_token = { ok: false, error: error.message }; console.log(`HF token preflight failed: ${error.message}`); }

  try {
    const modelResponse = await fetchWithDiagnostics(`https://huggingface.co/api/models/${encodeURIComponent(HF_PREFLIGHT_REPO).replace(/%2F/g, "/")}`, { method: "GET", headers: { Authorization: `Bearer ${HF_TOKEN}` } }, 30000);
    const body = await parseJsonResponse(modelResponse);
    summary.preflight.hf_lora_repo = { ok: modelResponse.ok, status: modelResponse.status, status_text: modelResponse.statusText, id: body.json?.id || "", private: Boolean(body.json?.private), error: modelResponse.ok ? "" : body.text.slice(0, 600) };
    console.log(`HF LoRA repo preflight: ${modelResponse.status} ${modelResponse.statusText}`);
  } catch (error) { summary.preflight.hf_lora_repo = { ok: false, error: error.message }; console.log(`HF LoRA repo preflight failed: ${error.message}`); }
}
...
async function requestFalImage(prompt, negativePrompt) {
  ...
  const response = await fetchWithDiagnostics(`https://fal.run/${FAL_MODEL}`, { method: "POST", headers: { Authorization: `Key ${FAL_KEY}`, "Content-Type": "application/json" }, body: JSON.stringify(falInput(prompt, negativePrompt)) }, FAL_TIMEOUT_MS);
  const body = await parseJsonResponse(response);
  if (!response.ok) throw new Error(`fal ${FAL_MODEL} failed ${response.status} ${response.statusText}: ${body.text.slice(0, 1000)}`);
...
async function requestHfFallbackImage(prompt, negativePrompt) {
  ...
  const response = await fetchWithDiagnostics(url, { method: "POST", headers: { Authorization: `Bearer ${HF_TOKEN}`, "Content-Type": "application/json", Accept: "image/png,image/jpeg,application/json" }, body: JSON.stringify(hfFallbackBody(prompt, negativePrompt)) }, HF_TIMEOUT_MS);
...
async function generateImage(prompt, negativePrompt) {
  try { return await requestFalImage(prompt, negativePrompt); }
  catch (falError) {
...
async function clearPanelImages(dirPath) { await ensureDir(dirPath); for (const name of PANEL_FILES) { try { await fs.unlink(path.join(dirPath, name)); } catch {} } }
...
async function writeSummary(dirA, dirB, summary) { await writeJson(path.join(dirA, "generation-summary.json"), summary); await writeJson(path.join(dirB, "generation-summary.json"), summary); }
async function writeImageGenerationStatus(date, summary) {
  const rels = [`image-manifests/${date}.json`, `daily/${date}.json`, "latest.json"];
  for (const rel of rels) {
    ...
    data.image_generation_status = {
      ...
      lora_file: summary.lora_file,
      lora_url: summary.lora_url,
      errors: summary.errors,
      checked_at: new Date().toISOString(),
    };
    data.post_ready_contract = { ...(data.post_ready_contract || {}), image_generation_passed: summary.generated_count === summary.requested_panels, generated_art_panel_count: summary.generated_count, fallback_template_panel_count: summary.fallback_template_panels.length, posting_allowed: (summary.generated_count === summary.requested_panels) && data.post_ready_contract?.posting_allowed !== false };
    if (summary.generated_count !== summary.requested_panels) data.post_ready_contract.posting_block_reasons = Array.from(new Set([...(data.post_ready_contract.posting_block_reasons || []), "image_generation_incomplete"]));
...
async function main() {
  ...
  await ensureDir(datedDir); await ensureDir(latestDir);
  const summary = { date, generated_at: new Date().toISOString(), route: FAL_KEY ? "fal-ai-z-image-turbo-lora-first" : "hf-fallback-only", fal_model: FAL_MODEL, hf_fallback_model: HF_FALLBACK_MODEL, lora_file: LORA_FILE, lora_url: LORA_URL, lora_url_mode: process.env.HF_LORA_URL ? "env_override" : "hf_repo_default", lora_auth: "not_sent_public_repo", trigger_word: TRIGGER, requested_panels: 6, generated_count: 0, generated_panels: [], fallback_template_panels: [], errors: [], preflight: {} };
  ...
  if (!promptPack) {
    ...
    await writeSummary(datedDir, latestDir, summary); await writeImageGenerationStatus(date, summary);
    console.log("Prompt pack missing. Image generation skipped; compositor will use fallback template art.");
    console.log("Image generation: 0/6 panels generated successfully"); console.log("Panels using generated art: []"); console.log(`Panels using fallback template: ${PANEL_FILES.join(", ")}`); return;
  }
  if (!FAL_KEY && !HF_TOKEN) {
  ...
  await clearPanelImages(datedDir); await clearPanelImages(latestDir);
  for (const panel of normalisePanels(promptPack)) {
    try {
      ...
      const datedOutput = path.join(datedDir, panel.image_name); const latestOutput = path.join(latestDir, panel.image_name);
      await savePng(result.bytes, datedOutput); await fs.copyFile(datedOutput, latestOutput);
      summary.generated_count += 1;
      summary.generated_panels.push({ panel_number: panel.panel_number, image_name: panel.image_name, provider: result.provider, model: result.model, output: `art-replacements/${date}/${panel.image_name}`, remote_url: result.remote_url || "", fal_error: result.fal_error || "" });
    } catch (error) {
  ...
  summary.fallback_template_panels = PANEL_FILES.filter((name) => !generatedNames.has(name)).map((name) => ({ panel_number: PANEL_FILES.indexOf(name) + 1, image_name: name, reason: "not generated" }));
  await writeSummary(datedDir, latestDir, summary); await writeImageGenerationStatus(date, summary);
...
main().catch((error) => { console.log(`Image generation recovered from unexpected error: ${error?.message || error}`); console.log("Image generation: 0/6 panels generated successfully"); console.log(`Panels using generated art: []`); console.log(`Panels using fallback template: ${PANEL_FILES.join(", ")}`); process.exit(0); });
